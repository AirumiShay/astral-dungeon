...
var undead = true
var radar = false
var target_enemy = Vector2(0,0)
var level = 1
var Damage = 10
signal died(id, level)
...
func _physics_process(delta): 
	if walk == true and radar == false:
#		get_parent().remove_child(self)
#		queue_free()

		move_and_slide((get_parent().get_node("Player").position - position).normalized() * speed / delta)#$AnimatedSprite.speed_scale / delta)
	if radar == true:
		$RADAR.radar_process(delta)
		move_and_slide((target_enemy - position).normalized() * speed / delta)#$AnimatedSprite.speed_scale / delta)
...
func die():	
	...
	if GlobalData.dead_enemy > 10:
		GlobalData.player_level = int(GlobalData.dead_enemy/Damage)
